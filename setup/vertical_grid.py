...
def build_vertical_grid(H, dzd, min_dz, depfac=1.03):
    '''
    Build a vertical grid using the method of Stewart et al. (2017),
    Input:
    - H: maximum ocean depth
    - dzd: maximum grid spacing
    - min_dz: minimum grid spacing
    - depfac: tuning parameter for hyperbolic tangent. <1 is shapr, 1 is neutral, >1 is gentle


    Returns three arrays
    - real_prop_z: location of z cell centres
    - real_delta_z: dz for each cell
    - super_vgrid: a MOM specific thing
    '''
...
# print "SUCCESS!! You now have a vertical grid of ", len(real_prop_z)-1, " levels with grid spacing ranging from ", real_delta_z[0]," to ", real_delta_z[-1]," written to files ",super_grid_filename," and ",regular_grid_filename
    ...
    return real_prop_z, real_delta_z, super_vgrid